...
void Calculator::parse_arithmetic(const std::string& expression, size_t& index,
                                  double x_value) {
  double value = 0.0;
...
void Calculator::parse_trigonometry(const std::string& expression,
                                    size_t& index) {
  switch (expression[index]) {
    case 's':
...
void Calculator::append_number(const std::string& expression, size_t& index,
                               double& value) {
  size_t next_index;
...
bool Calculator::all_arithmetic_operation(std::shared_ptr<Node> node) {
  return ((node->type >= Type::PLUS && node->type <= Type::MOD) ||
          (node->type == Type::UNARY_MINUS));
}
...
double Calculator::calc_sharting_yard() {
  ...
  while (current != nullptr) {
    if (current->type == Type::NUMBER) {
      calculationStack.push_right(current->data, current->type,
                                  current->priority);
    ...
    } else if ((current->type >= Type::COS && current->type <= Type::LOG) ||
               current->type == Type::UNARY_MINUS) {
...
void Calculator::arithmetic_calculation(Stack& calculationStack,
...
void Calculator::trigonometry_calculation(Stack& calculationStack,
                                          std::shared_ptr<Node> operation,
