...
Credit_output CreditCalculator::credit_calculator(const Credit_params& credit_params) {
  ...
  if (credit_params.annuity) {
    credit_result.monthly_payment =
        credit_params.sum_credit * monthly_rate / (1 - std::pow(1 + monthly_rate, -credit_params.term_credit));
