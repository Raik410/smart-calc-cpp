...
std::shared_ptr<Node> Stack::peek_left() const {
    return head;
}

std::shared_ptr<Node> Stack::peek_right() const {
    return tail;
}
...
void Stack::printStack() const {
    ...
    while (current != nullptr) {
        std::cout << "Data: " << current->data << " | Type: " << static_cast<int>(current->type) << " | Priority: " << static_cast<int>(current->priority) << std::endl;
