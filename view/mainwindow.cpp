...
void MainWindow::on_pushButton_graf_clicked() {
  ...
  for (X = xBegin; X <= xEnd; X += h) {
    x.push_back(X);
    y.push_back(calcController.calculateExpression(str_parser, Y * X));
  }
...
void MainWindow::calculateButton_clicked() {
  ...
  QString result_to_str = QString::number(calcController.calculateExpression(cInput, x_value), 'g', 15);
